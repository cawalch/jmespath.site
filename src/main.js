...
class App {
  ...
  determineTargetFile(hashFile, targetVersion) {
    ...
    if (!hashFile || !this.fileExistsInVersion(hashFile, targetVersion)) {
      if (hashFile && hashFile !== defaultFile) {
        console.warn(`Hash file '${hashFile}' not found for version '${targetVersion.id}'. Falling back to default '${defaultFile}'.`);
      } else if (!hashFile) {
  ...
  async loadInitialContent(forceReload = false) {
    ...
    const result = await this.loadContentAndScroll(targetVersionId, targetFile, targetSectionId, forceReload || this.isInitialLoad);
    ...
    if (selectedVersion) {
      ...
      this.loadContentAndScroll(selectedVersion.id, defaultFile, null, true).then((result) => {
        if (result.success) {
          ...
          this.currentVersionId = result.versionId; // Keep version if file failed
          this.currentFile = null;
          this.searchInput.disabled = true;
