...
ending index. The ending index is the first index which you do _not_
...
Also note in this example we are omitting the `start` and the `stop` value, which means to use the array's start for the `start` value and its end for the `stop` value.
...
[`slices`](https://jmespath.site/#spec-slices) in the specification.
...
describing people, and each array element is a JSON object with `first`, `last`, and `age` keys.
...
implementation could typically implement evaluating an object projection.
...
each contains an `instances` key whose value is an array, and we create a sub projection for each element in that array. Here's an example of that:
...
_before_ evaluating the RHS of a projection.
...
scenario is when you want to operate on the _result_ of a projection rather than projecting an expression onto each element in the array. For
...
This next concept,
[`multiselect lists`](https://jmespath.site/#spec-multiselectlist) and [`multiselect hashes`](https://jmespath.site/#spec-multiselecthash)
...
Below, the JMESPath expression finds all elements in `myarray` that
...
- See the [examples](https://jmespath.site/#examples). You'll see common
...
- To actually start using JMESPath, pick the language of your choice,
  and check out the [libraries](https://jmespath.site/#libraries) page for
  more information on using JMESPath in the language of your choice.
- Read the [JMESPath Spec](https://jmespath.site/#specification), which
